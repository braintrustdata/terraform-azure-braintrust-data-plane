...
```bash
helm install braintrust oci://public.ecr.aws/braintrust/helm/braintrust --namespace braintrust --create-namespace --version <version> --values helm-values.yaml
```
...
```bash
lb_ip_address=$(kubectl get service braintrust-api -n braintrust -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
az network lb list \
  --query "[?frontendIPConfigurations[?privateIPAddress=='$lb_ip_address']].{
     LB_IPAddress: frontendIPConfigurations[0].privateIPAddress, LB_FrontendIPConfigId: frontendIPConfigurations[0].id
  }" -o table
```
